var WebML = function(opts){
		...
		IndexStamp: createProgram(`#version 300 es
			...
			void main() {
				...
				if (uVertical) {
					...
					if (xy.y%2 == 1) {
						Activation = texelFetch(uInput,xy,0)+cos((float(xy.x))*Pi*pow(100.0,-float(xy.y>>1)/d));
						// Activation = texelFetch(uInput,xy,0)+vec4(cos((uTime+float(xy.x))*Pi));
					} else {
						Activation = texelFetch(uInput,xy,0)+sin((float(xy.x))*Pi*pow(100.0,-float(xy.y>>1)/d));
						// Activation = texelFetch(uInput,xy,0)+vec4(sin((uTime+float(xy.x))*Pi));
					}
				} else {
					highp float d = float(textureSize(uInput,0)[1]);
					if (xy.x%2 == 1) {
						Activation = texelFetch(uInput,xy,0)+cos((float(xy.y))*Pi*pow(100.0,-float(xy.x>>1)/d));
						// Activation = texelFetch(uInput,xy,0)+vec4(cos((uTime+float(xy.y))*Pi));
					} else {
						Activation = texelFetch(uInput,xy,0)+sin((float(xy.y))*Pi*pow(100.0,-float(xy.x>>1)/d));
		...
		IndexStampTimed: createProgram(`#version 300 es
			...
			void main() {
				...
				if (uVertical) {
					highp float d = float(textureSize(uInput,0)[0]);
					if (xy.y%2 == 1) {
						Activation = texelFetch(uInput,xy,0)+cos((uTime+float(xy.x))*Pi*pow(100.0,-float(xy.y>>1)/d));
						// Activation = texelFetch(uInput,xy,0)+vec4(cos((uTime+float(xy.x))*Pi));
					} else {
						Activation = texelFetch(uInput,xy,0)+sin((uTime+float(xy.x))*Pi*pow(100.0,-float(xy.y>>1)/d));
						// Activation = texelFetch(uInput,xy,0)+vec4(sin((uTime+float(xy.x))*Pi));
					}
				} else {
					highp float d = float(textureSize(uInput,0)[1]);
					if (xy.x%2 == 1) {
						Activation = texelFetch(uInput,xy,0)+cos((uTime+float(xy.y))*Pi*pow(100.0,-float(xy.x>>1)/d));
						// Activation = texelFetch(uInput,xy,0)+vec4(cos((uTime+float(xy.y))*Pi));
					} else {
						Activation = texelFetch(uInput,xy,0)+sin((uTime+float(xy.y))*Pi*pow(100.0,-float(xy.x>>1)/d));
						// Activation = texelFetch(uInput,xy,0)+vec4(sin((uTime+float(xy.y))*Pi));
					}
				}
			}
	...
	this.MultiHeadedRecurrentAttention = function(options) {
		...
		this.Dense2 = new self.DenseLayer({inputSize:this.attentionLayer.outputSize,ActivationFunction:'linear',outputs:options.queryKeyDims});
        ...
        this.call = function(Input) {
			this.Input = Input;
			var result = this.normLayer.call(this.Dense2.call(this.Dense1.call(this.Dense0.call(this.attentionLayer.call(Input,this.StateValue)))));
        ...
        this.backprop = function(grad,prevAct) {
			return this.attentionLayer.backprop(this.Dense0.backprop(this.Dense1.backprop(this.Dense2.backprop(this.normLayer.backprop(grad,"linear"),"relu"),"relu"),"linear"),prevAct)[0];
        }
		this.reset = function() {
	...
	this.FlattenLayer = function(options) {
		...
		this.call = function(Input) {
			...
			if (this.options.no3D) {
				this.Output.setSize([Input.size[0]*Input.size[1],1]);
			} else {
				this.Output.setSize([Input.size[0]*Input.size[1]*3,1]);
			}
    ...
    this.CreateLayers = function(options,inputSize) {
        ...
        for (var i=0; i<options.length; i++) {
            if (options[i].Convolutonal) {
                ...
                var layer = new self.SelfAttentionLayer({inputSize:size,depth:options[i].depth,queryKeyDims:options[i].queryKeyDims,valueDims:options[i].valueDims,mask:options[i].mask,ActivationFunction:act});
				p.push(layer.Parameters);
				s.push(layer.State);
				g.push(layer.Gradent);
				act = 0;
                ...
                var layer = new self.MultiHeadedSelfAttentionLayer({inputSize:size,heads:options[i].heads,queryKeyDims:options[i].queryKeyDims,valueDims:options[i].valueDims,mask:options[i].mask,ActivationFunction:act});
				p.push(layer.Parameters);
				g.push(layer.Gradent);
				act = 0;
				ParameterCount += layer.ParameterCount;
                ...
                var layer = new self.SimpleRecurrentAttention({inputSize:size,queryKeyDims:options[i].outputs,valueDims:options[i].outputs,ActivationFunction:act});
                Layers.push(layer);
				ParameterCount += layer.ParameterCount;
				p.push(layer.Parameters);
				s.push(layer.State);
                ...
                var layer = new self.MultiHeadedRecurrentAttention({inputSize:size,queryKeyDims:options[i].queryKeyDims,valueDims:options[i].valueDims,recurrency:options[i].recurrency,heads:options[i].heads,ActivationFunction:act});
                Layers.push(layer);
				ParameterCount += layer.ParameterCount;
				p.push(layer.Parameters);
				s.push(layer.State);
